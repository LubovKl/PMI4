...
class Admin_Catalog_Sections extends Controller {
    ...
    public function index() {
        if ($sections = $this->model->getSectionsList()) {
            $sections = $this->get_tree_for_array($section);
    ...
    public function get_section_list() {
        if ($sections = $this->model->getSectionsList()) {
            $this->get_tree_for_select ($section);
    ...
    protected function get_tree_for_array ($data, $level = 0, $pid = 0) {
        ...
        foreach ( $data as $row ) {
            if ($row['parent_id'] == $pid){
                ...
                $res =$this->get_tree_for_array($data, $level + 1,$row['id']);
                $row['count_sections'] = count($res);
                $result[] =$row;
...
//<option value="id" data-dept-level="d1"> - - - name </option>